...
/// Unit tests for HiveService encryption key management
/// Story 0.10 Phase 2: Encryption Configuration
///
/// Tests secure encryption key generation, storage, and deletion
void main() {
  group('HiveService Encryption Key Management', () {
    group('Key Derivation from Firebase Auth UID', () {
    ...
    group('Key Security Properties', () {
      test('should generate unpredictable keys (entropy check)', () {
        ...
        expect(
          uniqueBytes.length,
          greaterThan(10),
        ); // At least 10 different byte values
      });
    ...
    group('RGPD Compliance', () {
      test('key deletion should make data unrecoverable', () {
    ...
    group('Performance Requirements', () {
      test('key generation should be fast (< 100ms)', () {
      ...
      test('base64 encoding should be fast', () {
        final testKey = Uint8List.fromList(List<int>.generate(32, (i) => i));
        final stopwatch = Stopwatch()..start();
    ...
    group('Security Best Practices', () {
      test('key should have high entropy (randomness)', () {
        ...
        expect(
          uniqueBytes.length,
          greaterThan(15),
        ); // At least 15 unique values out of 32
      });

      test('key should not be predictable from UID', () {
        ...
        expect(
          differentBytes,
          greaterThan(20),
        ); // At least 20 bytes different out of 32
      });
      ...
      test('key should not contain easily guessable patterns', () {
        ...
        final isIncreasing = List.generate(
          31,
          (i) => key[i] < key[i + 1],
        ).every((x) => x);
        final isDecreasing = List.generate(
          31,
          (i) => key[i] > key[i + 1],
        ).every((x) => x);
